...
const projectOptions = [
  { id: 'p1', title: 'Capstone Platform Modernization' },
  { id: 'p2', title: 'AI Monitoring Dashboard' },
];

const milestoneOptions = [
  { id: 'm1', title: 'Prototype Submission' },
  { id: 'm2', title: 'User Testing' },
  { id: 'm3', title: 'Final Report' },
];
...
export default function SubmissionForm({ onSubmit, isSubmitting }: { onSubmit: (payload: any) => Promise<void>; isSubmitting: boolean }) {
  ...
  const [milestoneId, setMilestoneId] = useState(milestoneOptions[0].id);
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    if (!milestoneId) nextErrors.milestoneId = 'Select a milestone.';
    ...
    const payload = {
      ...
      content: details,
      evidenceType,
      fileUrl: file ? URL.createObjectURL(file) : sourceUrl || null,
      metadata,
      status,
    };
    ...
    <form onSubmit={handleSubmit} className="rounded bg-white p-6 shadow">
      <div className="mb-6 flex flex-col gap-4 md:flex-row md:items-end">
        <div className="flex-1">
          ...
          <select value={projectId} onChange={(e) => setProjectId(e.target.value)} className="mt-1 w-full rounded border px-3 py-2">
        ...
        <div className="flex-1">
          ...
          <select value={milestoneId} onChange={(e) => setMilestoneId(e.target.value)} className="mt-1 w-full rounded border px-3 py-2">
            {milestoneOptions.map((milestone) => (
              <option value={milestone.id} key={milestone.id}>{milestone.title}</option>
            ))}
          </select>
          {errors.milestoneId && <p className="mt-1 text-sm text-red-600">{errors.milestoneId}</p>}
        </div>
      </div>
      ...
      <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <button type="submit" disabled={isSubmitting} className="rounded bg-blue-600 px-5 py-2 text-white hover:bg-blue-700 disabled:opacity-50">
